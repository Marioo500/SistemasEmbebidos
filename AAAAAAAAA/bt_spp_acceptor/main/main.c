/*
    Santos Mendez Daniel        01262465
    Moreno Lopez Luis Mario     01271304
    Gonzalez Zepeda Alejandro   01250255
*/
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param){
    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        esp_log_buffer_char("Hola",param->data_ind.data,param->data_ind.len);
        uint32_t hola = param->data_ind.data; //Recibe la cadena
        int a = param->data_ind.len;
        token = strtok(hola, ",");
        uartPuts(PC_UART_PORT, buffer);
        while (token != NULL)
        {
            token = strtok(NULL, ",");
            uartPuts(UART2, token); //Se manda por UART
            delay_ms(250);
        }
